...
from .client import DotCraftError, JsonRpcError
from .models import (
...
class InitializationError(DotCraftError):
    ...
    def __init__(self, message: str) -> None:
        super().__init__("initializationFailed", message)


class ProtocolViolationError(DotCraftError):
    def __init__(self, message: str) -> None:
        super().__init__("protocolViolation", message)


class TurnInProgressError(JsonRpcError):
    ...
    def __init__(self, message: str = "A turn is already running on this thread.") -> None:
        super().__init__(ERR_TURN_IN_PROGRESS, message, code="turnInProgress")


class ThreadNotFoundError(JsonRpcError):
    """The specified thread does not exist."""

    def __init__(self, message: str = "Thread not found.") -> None:
        super().__init__(ERR_THREAD_NOT_FOUND, message, code="threadNotFound")


class ThreadNotActiveError(JsonRpcError):
    """The thread cannot accept turns because it is paused or archived."""

    def __init__(self, message: str = "Thread is not active.") -> None:
        super().__init__(ERR_THREAD_NOT_ACTIVE, message, code="threadNotActive")
...
class TurnFailedError(DotCraftError):
    ...
    def __init__(self, message: str, thread_id: str, turn_id: str | None = None) -> None:
        super().__init__("turnFailed", message)
...
class TurnCancelledError(DotCraftError):
    ...
    def __init__(self, thread_id: str, turn_id: str | None = None, reason: str | None = None) -> None:
        super().__init__("turnCancelled", reason or "The turn was cancelled.")
        self.thread_id = thread_id
        self.turn_id = turn_id


class ApprovalTimeoutError(JsonRpcError):
    """The client did not answer an approval request in time."""

    def __init__(self, message: str = "Approval timed out.") -> None:
        super().__init__(ERR_APPROVAL_TIMEOUT, message, code="approvalTimeout")


class RunDisconnectedError(DotCraftError):
    def __init__(self, message: str = "Run disconnected before completion.") -> None:
        super().__init__("runDisconnected", message)

...
export function runModuleConformanceSuite(
  ...
  test(`${packageName} manifest conformance`, async () => {
    ...
    assert.equal(mod.manifest.channelContractVersion, CHANNEL_CONTRACT_VERSION);
    assert.ok(
      Array.isArray(mod.manifest.supportedChannelProtocolVersions) && mod.manifest.supportedChannelProtocolVersions.length > 0,
      "supportedChannelProtocolVersions must be non-empty",
    );

...
import type { SessionThread, SessionTurn } from "@dotcraft/sdk/contracts";
import { makeThread, makeTurn } from "./test-contract-fixtures.js";
import { shouldFlushSegmentOnItemStarted } from "./segmentBoundaries.js";
import type { Transport } from "@dotcraft/sdk/wire";
...
test("processMessage enqueues bound social input instead of streaming a turn", async () => {
  ...
  assert.deepEqual(enqueueParams, {
    ...
    sender: { senderId: "u", senderName: "Tester", senderRole: "user", groupId: "group-123" },
  });
});
...
test("processMessage falls back to normal channel routing when social binding resolve fails", async () => {
  ...
  (adapter as unknown as {
    getOrCreateThread: (...args: unknown[]) => Promise<SessionThread>;
  }).getOrCreateThread = async () => makeThread("thread-fallback-1", "active");
  client.turnStart = async () => {
    turnStarted = true;
    return makeTurn("turn-fallback-1", "thread-fallback-1", "running");
  };
  client.streamEvents = () =>
    (async function* () {
      yield {
...
test("ChannelAdapter lets explicit sender override thread identity and omit implicit channel context groupId", async () => {
  ...
  client.turnStart = async (_threadId: string, _input: unknown, sender: unknown) => {
    senders.push(sender);
    return makeTurn(`turn-${senders.length}`, "thread-1", "running");
  };
  client.streamEvents = () =>
    (async function* () {
      yield {
...
test("getOrCreateThread forces a fresh thread after newThread", async () => {
  ...
  client.threadList = async () => {
    threadListCalls += 1;
    return [makeThread("thread-hidden", "active")];
  };
  client.threadArchive = async (threadId: string) => {
    archived.push(threadId);
  };
  ...
  client.threadStart = async () => {
    threadStartCalls += 1;
    return makeThread("thread-fresh", "active");
  };

  const created = await (adapter as unknown as {
    getOrCreateThread: (identityKey: string, userId: string, channelContext: string, workspacePath: string) => Promise<SessionThread>;

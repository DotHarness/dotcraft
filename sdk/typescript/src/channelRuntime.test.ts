...
class FakeRuntimeClient {
  ...
  async threadResume(threadId: string): Promise<Thread> {
    ...
    const resumed = new Thread(threadId, "active");
    this.threads.set(threadId, resumed);
    return resumed;
  }

  async threadList(_params: Record<string, unknown>): Promise<Thread[]> {
    this.events.push("list");
    return this.listResult;
  }

  async threadStart(_params: Record<string, unknown>): Promise<Thread> {
    this.events.push("start");
    const thread = new Thread(this.nextStartedId, "active");
...
test("ThreadResolver resolves cache, list reuse, fresh reset, and not-active recovery", async () => {
  ...
  fake.threads.set("cached", new Thread("cached", "active"));
  fake.threads.set("paused", new Thread("paused", "paused"));
  fake.threads.set("listed", new Thread("listed", "active"));
  const eventsSeen: string[] = [];
  ...
  fake.listResult = [new Thread("listed", "active")];
  ...
  fake.listResult = [new Thread("cached", "active"), new Thread("old-paused", "paused")];
  ...
  fake.listResult = [new Thread("recover-paused", "paused")];
...
test("CommandRouter handles expanded prompts, reset payloads, and DotCraftError delivery", async () => {
  const fake = new FakeRuntimeClient();
  ...
  fake.commandError = new DotCraftError(-32000, "Command failed");
  assert.deepEqual(await router.routeForTurn({
    identityKey: "u:c",

...
dotnet build (Join-Path $toolRoot 'src/DotCraft.Unity.csproj') --nologo -v:q
...
foreach ($name in @('DotCraft.Unity.dll','DotCraft.Unity.deps.json','Microsoft.CodeAnalysis.dll','Microsoft.CodeAnalysis.CSharp.dll')) {
    Copy-Item -LiteralPath (Join-Path $toolRoot "src/bin/Debug/net10.0/$name") -Destination (Join-Path $managedOutput $name) -Force
}

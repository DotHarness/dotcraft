...
- `Screenshot`: `{ width, height }`. Coordinates are pixels of this image.
...
| `click({ window, x, y, click_count = 1, mouse_button = "left" })` | Clicks a screenshot pixel. `mouse_button` is `left`, `right` or `middle`; `click_count` is 1 to 3. |

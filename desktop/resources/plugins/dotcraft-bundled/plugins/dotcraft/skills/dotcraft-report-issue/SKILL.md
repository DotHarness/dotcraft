...
Use this skill to convert the output of `dotcraft-error-diagnosis` or a user's bug description into a single, high-signal GitHub issue for the **DotHarness/dotcraft** repository. The goal is a concise report a maintainer can triage: clear title, public reproduction context, non-sensitive diagnostics, and environment.
...
Typical flow: `dotcraft-error-diagnosis` runs first and returns Finding / Evidence / Fix / Residual risk. Map that directly:

...
- **Placement:** a page's primary figure goes directly after the intro block, above the first `##` heading; a section-specific figure follows its section heading. Avoid adding media that serves the same purpose. An overview diagram and step-by-step screenshots can complement each other; adding screenshots does not imply replacing an existing diagram.

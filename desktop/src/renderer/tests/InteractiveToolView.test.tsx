...
function mountFrame(): { frameWindow: Window; postSpy: ReturnType<typeof vi.spyOn> } {
...
describe('InteractiveToolView', () => {
  ...
  it('forwards tools/call to ui/tool/call and returns the result', async () => {
    ...
    const { frameWindow, postSpy } = mountFrame()
    dispatch(frameWindow, { id: 5, method: 'tools/call', params: { name: 'ListItems', arguments: { q: 'x' } } })
  ...
  it('surfaces a gated tool-call rejection as an error to the UI', async () => {
    ...
    const { frameWindow, postSpy } = mountFrame()
    dispatch(frameWindow, { id: 7, method: 'tools/call', params: { name: 'CreateCard' } })
  ...
  it('forwards ui/open-link and opens the cleared url', async () => {
    ...
    const { frameWindow } = mountFrame()
    dispatch(frameWindow, { id: 6, method: 'ui/open-link', params: { url: 'https://oratorio.example/board/1' } })
  ...
  it('forwards ui/update-model-context with the item-derived sourceCallId', async () => {
    const { frameWindow } = mountFrame()
    dispatch(frameWindow, { id: 8, method: 'ui/update-model-context', params: { title: 'state', content: 'selected card-7' } })
  ...
  it('injects a visible turn for ui/message and rate-limits a rapid second message', async () => {
    setWorkspace()
    const { frameWindow } = mountFrame()
    dispatch(frameWindow, { id: 10, method: 'ui/message', params: { role: 'user', content: 'discuss this' } })
    ...
    dispatch(frameWindow, { id: 11, method: 'ui/message', params: { content: 'again immediately' } })
  ...
  it('forwards ui/set-widget-state to item/widget-state/set keyed by callId', async () => {
    const { frameWindow } = mountFrame()
    dispatch(frameWindow, { id: 12, method: 'ui/set-widget-state', params: { widgetState: { tab: 3 } } })
  ...
  it('arbitrates ui/request-display-mode and grants fullscreen', () => {
    const { frameWindow, postSpy } = mountFrame()
    dispatch(frameWindow, { id: 20, method: 'ui/request-display-mode', params: { mode: 'fullscreen' } })
  ...
  it('rejects an invalid display mode', () => {
    const { frameWindow, postSpy } = mountFrame()
    dispatch(frameWindow, { id: 21, method: 'ui/request-display-mode', params: { mode: 'huge' } })

...
describe('settings panel shell', () => {
  ...
  it('defines the unified English and Chinese copy keys', () => {
    const catalogSource = readSrcFile('shared/locales/catalog.ts')

    expect(catalogSource).toContain("'settings.general.description': 'Set language, theme, notifications, and workspace permissions.'")
    expect(catalogSource).toContain("'settings.general.description': '设置界面语言、主题、通知和工作区权限。'")
    expect(catalogSource).toContain("'settings.connection.description': 'Choose how DotCraft connects to this workspace.'")
    expect(catalogSource).toContain("'settings.connection.description': '选择 DotCraft 如何连接当前工作区。'")
    expect(catalogSource).toContain("'settings.usage.dashboardUnavailable': 'No hosted Dashboard URL is available for this workspace yet.'")
    expect(catalogSource).toContain("'settings.usage.dashboardUnavailable': '当前工作区暂未提供服务器托管的 Dashboard 地址。'")
    expect(catalogSource).toContain("'settings.group.workspaceSettings': 'Workspace settings'")
    expect(catalogSource).toContain("'settings.group.workspaceSettings': '工作区设置'")
    expect(catalogSource).toContain("'settings.group.profileDetails': 'Profile details'")
    expect(catalogSource).toContain("'settings.group.profileDetails': '助手详情'")
  })
})

...
describe('PluginsView local plugin visibility', () => {
  ...
  it('prefers an enabled plugin-id skill for try in chat', async () => {
    ...
    appServerSendRequest.mockImplementation(async (method: string) => {
      if (method === 'plugin/list') return { plugins: [doctorPlugin], diagnostics: [] }
      if (method === 'plugin/view') return { plugin: doctorPlugin }
      return {}
    })
  ...
  it('uses the only enabled skill for try in chat when no entry skill exists', async () => {
    ...
    appServerSendRequest.mockImplementation(async (method: string) => {
      if (method === 'plugin/list') return { plugins: [singleSkillPlugin], diagnostics: [] }
      if (method === 'plugin/view') return { plugin: singleSkillPlugin }
      return {}
    })
  ...
  it('does not choose an arbitrary skill for a multi-skill plugin without an entry skill', async () => {
    ...
    appServerSendRequest.mockImplementation(async (method: string) => {
      if (method === 'plugin/list') return { plugins: [multiSkillPlugin], diagnostics: [] }
      if (method === 'plugin/view') return { plugin: multiSkillPlugin }
      return {}
    })

...
export function TeamsView(): JSX.Element {
  ...
  async function createMission(): Promise<void> {
    ...
    setError(null)
    try {
      const result = await window.api.appServer.sendRequest('teams/mission/create', {
        title: missionTitle,
  ...
  async function archiveMission(mission: Mission): Promise<void> {
    ...
    setError(null)
    try {
      const result = await window.api.appServer.sendRequest('teams/mission/archive', {
  ...
  async function cancelMissionFromDiscard(card: BoardCard, dragState: DragState): Promise<void> {
    ...
    setError(null)
    try {
      const result = await window.api.appServer.sendRequest('teams/mission/cancel', {
  ...
  async function openThread(params: OpenThreadParams): Promise<void> {
    ...
    setError(null)
    try {
      const result = await window.api.appServer.sendRequest('teams/member/openThread', params) as { threadId?: string }
  ...
  useEffect(() => {
    ...
    const unsubscribe = window.api.appServer.onNotification((payload) => {
      if (
        ...
        payload.method === 'thread/runtimeChanged' ||
        payload.method.startsWith('turn/')
      ) {
        void refresh()
      }
    })

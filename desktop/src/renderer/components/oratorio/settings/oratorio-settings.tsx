...
function useWorkspaceBindings(): { options: WorkspaceBindingOption[]; loading: boolean } {
  const projects = useWorkspaceProjectsStore((state) => state.projects)
  const foregroundWorkspacePath = useWorkspaceProjectsStore((state) => state.foregroundWorkspacePath)
  ...
  useEffect(() => {
    ...
    void window.api.workspace.getProjects()
      .then((payload) => {
        if (active) useWorkspaceProjectsStore.getState().setPayload(payload)
      })
      .catch(() => {})
      .finally(() => {
  ...
  const options = useMemo(() => {
    const local = projects.filter((project) => project.kind === 'local')
    const foregroundIndex = local.findIndex((project) => project.path === foregroundWorkspacePath)
    const ordered = foregroundIndex > 0
      ? [local[foregroundIndex], ...local.slice(0, foregroundIndex), ...local.slice(foregroundIndex + 1)]
      : local
    return ordered.map((project) => ({ value: project.path, label: project.path }))
  }, [foregroundWorkspacePath, projects])
...
function useSettingsErrorToast(): NotifySettingsError {
  ...
  return useCallback((message: string, retry?: () => void) => {
    if (activeToastRef.current) removeToast(activeToastRef.current)
    let toastId = ''
    toastId = showToast({
      message,
      type: 'error',
      onExpire: () => {
        if (activeToastRef.current === toastId) activeToastRef.current = null
      },
      ...(retry ? {
        action: {
          label: t('retry'),
...
export function OratorioSettingsPanel({ view = 'root', serviceError = false, readOnly = false, onViewChange }: { view?: OratorioSettingsView; serviceError?: boolean; readOnly?: boolean; onViewChange: (view: OratorioSettingsView) => void }): ReactNode {
  ...
  const startProjectSync = (project: OratorioProjectConfig, queued = false): void => {
    ...
    void window.api.oratorio.request({
...
function ProjectSettings({ project, workspaceOptions, workspaceLoading, sync, controller, onSync, onBack, onRemoved }: { project: OratorioProjectConfig; workspaceOptions: WorkspaceBindingOption[]; workspaceLoading: boolean; sync: ProjectSyncState; controller: Controller; onSync: () => void; onBack: () => void; onRemoved: () => void }) {
  const t = useOratorioSettingsT(); const [confirming, setConfirming] = useState(false); const Icon = project.provider === 'github' ? GithubGlyph : GitlabGlyph

...
function ReviewDraftCard({ draft, busy, mutate }: { draft: ReviewDraftDto; busy: string | null; mutate: Mutate }) {
  ...
  return <article className="ora-draft-card"><header><span className="ora-status">{draft.status}</span><strong>{draft.majorCount} major · {draft.minorCount} minor · {draft.suggestionCount} suggestions</strong></header><Textarea value={summary} readOnly={draft.status !== 'draft' && draft.status !== 'publishFailed'} onChange={(event) => setSummary(event.target.value)} aria-label="Review summary" />
...
function DetailSection({ title, description, action, children }: { title: string; description?: string; action?: ReactNode; children: ReactNode }) { return <section className="ora-detail-section"><header><span><h2>{title}</h2>{description ? <p>{description}</p> : null}</span>{action}</header><div>{children}</div></section> }

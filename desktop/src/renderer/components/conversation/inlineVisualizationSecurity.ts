...
:root{color-scheme:light dark;--background:light-dark(rgb(255 255 255),rgb(24 24 24));--foreground:light-dark(rgb(26 28 31),rgb(255 255 255));--card:color-mix(in srgb,var(--foreground) 5%,transparent);--card-foreground:var(--foreground);--primary:light-dark(rgb(51 156 255),rgb(131 195 255));--primary-foreground:light-dark(white,rgb(13 13 13));--secondary:light-dark(rgb(255 255 255 / 96%),rgb(54 54 54 / 96%));--secondary-foreground:var(--foreground);--muted:color-mix(in srgb,var(--foreground) 10%,transparent);--muted-foreground:color-mix(in srgb,var(--foreground) 52%,transparent);--accent:color-mix(in srgb,var(--primary) 14%,transparent);--accent-foreground:var(--primary);--destructive:light-dark(rgb(226 85 7),rgb(255 133 73));--border:color-mix(in srgb,var(--foreground) 9%,transparent);--input:color-mix(in srgb,var(--foreground) 13%,transparent);--ring:var(--primary);--font-size-base:14px;--viz-container-width:100%;--viz-series-1:var(--primary);--viz-series-2:light-dark(rgb(243 136 59),rgb(245 154 86));--viz-series-3:light-dark(rgb(93 201 119),rgb(116 213 139));--viz-series-4:light-dark(rgb(235 119 177),rgb(240 143 192));--viz-series-5:light-dark(rgb(155 121 236),rgb(170 145 239));--viz-series-6:light-dark(rgb(58 185 177),rgb(90 203 194));font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;font-size:max(11px,var(--font-size-base));line-height:1.45;color:var(--foreground);background:transparent}
:root[data-theme=light]{color-scheme:light}:root[data-theme=dark]{color-scheme:dark}*{box-sizing:border-box}html,body{margin:0;padding:0;background:transparent}body{padding:5px;overflow:hidden}img,canvas,svg,video{display:block;max-width:100%;height:auto}button,input,select,textarea{font:inherit}.card{min-width:0;padding:12px;overflow:hidden;border:1px solid var(--border);border-radius:16px;color:var(--card-foreground);background:var(--card)}.viz-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(min(180px,100%),1fr));gap:10px}.viz-row,.viz-controls{display:flex;flex-wrap:wrap;align-items:center;gap:8px}.viz-stat{display:flex;min-width:0;flex-direction:column;gap:2px}.viz-stat-value{font-size:1.4em;font-weight:600;line-height:1.2}.viz-badge{display:inline-flex;align-items:center;padding:3px 8px;border-radius:999px;color:var(--accent-foreground);background:var(--accent)}.text-small{font-size:max(11px,calc(var(--font-size-base) - 2px))}.text-muted{color:var(--muted-foreground)}.text-center{text-align:center!important}.text-end{text-align:end!important}.sr-only{position:absolute;width:1px;height:1px;padding:0;overflow:hidden;clip:rect(0,0,0,0);white-space:nowrap;border:0}.btn{display:inline-flex;min-height:28px;align-items:center;justify-content:center;gap:4px;padding:0 8px;border:1px solid var(--input);border-radius:8px;color:var(--secondary-foreground);background:var(--secondary);cursor:pointer}.btn-primary{border-color:transparent;color:var(--primary-foreground);background:var(--primary)}.btn-ghost{border-color:transparent;color:var(--muted-foreground);background:transparent}.btn:disabled{cursor:not-allowed;opacity:.55}.btn:focus-visible,.form-control:focus-visible,.form-select:focus-visible,.form-range:focus-visible,.form-check:focus-visible{outline:2px solid var(--ring);outline-offset:2px}.form-label{display:block;margin-block-end:4px;color:var(--foreground)}.form-control,.form-select{display:block;width:100%;min-height:28px;padding:0 8px;border:1px solid var(--input);border-radius:8px;color:var(--foreground);background:var(--secondary)}textarea.form-control{padding-block:6px;resize:vertical}.form-range{display:block;width:100%;height:28px;accent-color:var(--primary)}.form-check{width:16px;height:16px;margin:0;accent-color:var(--primary)}.form-switch{appearance:none;width:32px;height:18px;padding:2px;border:1px solid var(--input);border-radius:999px;background:var(--muted);cursor:pointer}.form-switch:before{display:block;width:12px;height:12px;border-radius:50%;background:var(--foreground);content:"";transition:transform .15s ease}.form-switch:checked{background:var(--primary)}.form-switch:checked:before{transform:translateX(14px);background:var(--primary-foreground)}.table-responsive{width:100%;overflow-x:auto}.table{width:100%;border-collapse:collapse;color:var(--foreground)}.table th,.table td{padding:8px 16px 8px 0;border-bottom:1px solid var(--border);text-align:start;vertical-align:top}.table th{font-weight:600}[data-tooltip]{position:relative}[data-tooltip]:hover:after,[data-tooltip]:focus-visible:after{position:absolute;z-index:20;inset-block-end:calc(100% + 6px);inset-inline-start:50%;max-width:min(260px,80vw);padding:5px 8px;border:1px solid var(--border);border-radius:6px;color:var(--foreground);background:var(--secondary);box-shadow:0 4px 16px rgb(0 0 0 / 18%);content:attr(data-tooltip);font-size:11px;line-height:1.3;white-space:normal;transform:translateX(-50%);pointer-events:none}
@media(max-width:736px){body{padding:4px}.card{padding:10px;border-radius:12px}.viz-grid{grid-template-columns:repeat(auto-fit,minmax(min(150px,100%),1fr))}.table th,.table td{padding:7px 10px 7px 0}}
...
export function buildInlineVisualizationDocument(fragment: string, theme: 'light' | 'dark', locale: string, viewId: string): string {
  const csp = [
    "default-src 'none'",
  ...
  const bridge = String.raw`(() => {
    ...
    addEventListener('message', event => {
      ...
      if (message?.method === 'visualization/theme') document.documentElement.dataset.theme = message.params?.theme === 'light' ? 'light' : 'dark';
      if (message?.method === 'visualization/context') {
        ...
        if (typeof value.accent === 'string' && value.accent) document.documentElement.style.setProperty('--primary', value.accent);
      }

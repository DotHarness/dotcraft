...
export function SettingsSelect<T extends string = string>({
  ...
  const applyValueChange = useCallback((nextValue: string) => {
    const nextOption = options.find((option) => option.value === nextValue)
    if (nextOption && !nextOption.disabled) {
      const result = onValueChange(nextOption.value)
      void Promise.resolve(result)
        .then((applied) => {
          if (applied === false && triggerRef.current) {
            triggerRef.current.value = value
          }
        })
        .catch(() => {
          if (triggerRef.current) triggerRef.current.value = value
        })
    }
  ...
  useEffect(() => {
    ...
    function handleNativeChange(event: Event): void {
      applyValueChange((event.currentTarget as HTMLButtonElement).value)
    }
  ...
  }, [activeIndex, ariaLabel, listboxId, onValueChange, open, options, position, value])
...
function firstEnabledIndex<T extends string>(options: ReadonlyArray<SettingsSelectOption<T>>): number {
  return options.findIndex((option) => !option.disabled)
}

function lastEnabledIndex<T extends string>(options: ReadonlyArray<SettingsSelectOption<T>>): number {

...
import { Button, IconButton, InlineDiff, Textarea } from './ui'
import { showOratorioToast } from './runtime'
...
export function OratorioTaskDetail({ task, initialStage = 'review', initialFocus, onBack, onOpenThread, onTaskChange, onStageChange }: {
  ...
  async function mutate(key: string, operation: Promise<ItemDetailResponse>, message: string): Promise<void> {
    ...
    try {
      ...
      showOratorioToast({ message, tone: 'success' })
    } catch (error) {
      showOratorioToast({ message: error instanceof Error ? error.message : 'Oratorio rejected the action.', tone: 'error' })
    } finally {
      setBusy(null)
    }
  }

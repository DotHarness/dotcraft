...
export function OratorioQuickView({
  ...
  function runAction(action: QuickActionId, note?: string, confirmed = false): void {
    ...
    if (!onAction) {
      showOratorioToast({ message: t('actionFailed'), tone: 'error' })
      return
    }
    ...
    void onAction(action, note).then((next) => {
      ...
      setCancelRunNote('')
      onTaskChange(next)
      showOratorioToast({ message: actionMessageFor(action), tone: 'success' })
    }).catch((error) => {
      showOratorioToast({ message: error instanceof Error ? error.message : t('actionFailed'), tone: 'error' })

...
describe('desktop pet native ownership', () => {
  ...
  it('relays dictation only for the action the displayed voice state allows', () => {
    const pet = detach()
    const voiceEvents = (): PetEvent[] => events(owner).filter((event) => event.type === 'voice')
    send(pet, { type: 'voice', action: 'start' })
    send(owner, { type: 'snapshot', snapshot: { ...snapshot, voice: 'idle' } })
    send(pet, { type: 'voice', action: 'stop' })
    send(pet, { type: 'voice', action: 'toString' } as any)
    send(owner, { type: 'voice', action: 'start' } as any)
    expect(voiceEvents()).toEqual([])
    send(pet, { type: 'voice', action: 'start' })
    send(owner, { type: 'snapshot', snapshot: { ...snapshot, voice: 'recording' } })
    send(pet, { type: 'voice', action: 'cancel' })
    expect(voiceEvents()).toEqual([{ type: 'voice', action: 'start' }, { type: 'voice', action: 'cancel' }])
    send(owner, { type: 'snapshot', snapshot: { ...snapshot, voice: 'loud' } as any })
    expect(events(pet).filter((event) => event.type === 'snapshot').at(-1)).toEqual({ type: 'snapshot', snapshot: { ...snapshot, voice: 'recording' } })
  })

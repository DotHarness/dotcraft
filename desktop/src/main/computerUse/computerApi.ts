...
export class ComputerApi {
  ...
  private async getWindowState(input: Args): Promise<unknown> {
    ...
    if (typeof state.snapshot_id === 'string') this.snapshots.set(hwnd, state.snapshot_id)
    ...
    const screenshots = includeScreenshot && typeof state.capture_id === 'string'
      ? [{ id: state.capture_id, width: state.screenshot_width, height: state.screenshot_height }]
      : []
    return {
      window,
      screenshots,
  ...
  private async click(input: Args): Promise<void> {
    ...
    const hwnd = windowHandle(input)
    const location = input.element_index !== undefined
      ? this.elementTarget(hwnd, input)
      : {
          x: requiredNumber(input, 'x'),
          y: requiredNumber(input, 'y'),
          ...(typeof input.screenshotId === 'string' ? { capture_id: input.screenshotId } : {})
        }

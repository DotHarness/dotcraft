...
For remote execution, mark a generated tool or `[ToolDeclaration]` with `[ToolRpc]`. Hand-written definitions use `RemoteToolMetadata.RpcEligibleAnnotation` set to JSON `true`. The plugin must activate with the execution host's tool services and dependency exports, without requiring Agent, Session, or provider services. See [remote tool calls](../architecture/remote-tool-host#skill-and-plugin-resources) for bundle preparation and remote authorization.

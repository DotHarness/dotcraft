...
- **Auth**: WebSocket `?token=` query string ([details](./appserver.md))

See [AppServer Protocol](./appserver-protocol.md) and [AppServer Mode](./appserver.md).

## Hub

Each user has one [Hub](./hub.md) on the machine. Hub starts/reuses one AppServer per workspace and maintains discovery info and locks under `~/.craft/hub/`. Desktop and TUI use Hub by default. Bypass Hub for remote, CI, bots, or protocol debugging — use [AppServer Mode](./appserver.md).
...
- SubAgents: `native` and `cli-oneshot` runtimes (see [SubAgents](../features/subagents.md))
...
Full field reference: [Configuration Reference](./configuration.md). How fields take effect (immediate / subsystem restart / AppServer restart): [Settings Lifecycle](./settings-lifecycle.md).

## Related

- [Configuration Reference](./configuration.md)
- [AppServer Protocol](./appserver-protocol.md) / [AppServer Mode](./appserver.md)
- [Hub Protocol](./hub-protocol.md) / [Hub Local Coordination](./hub.md)
- [SDK Overview](./sdk.md) · [TypeScript SDK](./sdk-typescript.md) · [.NET SDK](./sdk-dotnet.md) · [Python SDK](./sdk-python.md)

...
App Binding is the platform flow for connecting an **external native app** to DotCraft and granting one thread access to app-owned tools — without the app handing over its accounts, authorization, or high-risk operations. This page is the builder's guide; for the user-facing experience see [App Binding](../features/app.md).
...
DotCraft launches your registered protocol (it never spawns an executable). The app parses the URL, inspects the request over the short-lived handoff endpoint, accepts it, and attaches tools. With the [.NET SDK](./sdk-dotnet.md):
...
The full RPC surface is in the [.NET SDK](./sdk-dotnet.md) and the [SDK overview](./sdk.md). The same flow is available to any language over the [AppServer Protocol](./appserver-protocol.md).
...
For typed parameters and results, use the [.NET SDK](./sdk-dotnet.md) or the [AppServer Protocol](./appserver-protocol.md). Persisted App Binding state lives at `.craft/app-bindings/state.json`.

## See Also

- [App Binding](../features/app.md) — the user-facing feature.
- [SDKs](./sdk.md) and [.NET SDK](./sdk-dotnet.md) — client libraries and App Binding helpers.
- [AppServer Protocol](./appserver-protocol.md) — the wire contract behind every SDK.
- [Plugins & Tools](../features/plugins-tools.md) — how plugins package apps.

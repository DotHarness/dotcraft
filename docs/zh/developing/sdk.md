...
需要客户端库时使用 SDK。需要实现新传输、调试 wire protocol，或完全控制 JSON-RPC 消息时，直接使用 [AppServer 协议](./appserver-protocol.md)。
...
| SDK | 包 | 适用场景 | 说明 |
|-----|----|----------|------|
| [TypeScript](./sdk-typescript.md) | `@dotcraft/sdk` | Node.js 应用、一方 TypeScript 渠道模块、高层 run helper。 | 当前高层应用侧能力最完整，包含 `run()` / `runStreamed()`、回调处理、Hub helper 和 channel runtime 组件。 |
| [.NET](./sdk-dotnet.md) | `DotCraft.Sdk` | 原生 App、App Binding 集成、C# 工具和 typed AppServer client。 | 提供 local/remote 连接、线程/轮次/模型 wrapper、Runtime Dynamic Tools、App Binding helper 和 raw notification 访问。 |
| [Python](./sdk-python.md) | `dotcraft_wire` | Python 外部渠道适配器和 wire protocol 客户端。 | 保留为 Python adapter/wire SDK，支持 stdio/WebSocket、审批、投递、channel tools，并包含 Telegram 参考适配器。 |
...
- [AppServer 协议](./appserver-protocol.md)
- [Hub 本地协调](./hub.md)

...
> [!NOTE]
> 日常 Desktop / TUI 走 [Hub 本地协调](./hub.md)，本页只在你需要手动管理 AppServer 时使用。
...
- [配置参考](./configuration.md) — `AppServer.*` / `CLI.*` 字段
- [AppServer 协议](./appserver-protocol.md) — 客户端协议概览
- [Hub 本地协调](./hub.md) — 日常 Desktop / TUI 走的路径
- [统一会话核心](../features/session-core.md) — Thread / Turn / Item 模型

...
插件安装、移除或启用状态变化会广播 `workspace/configChanged`，`regions: ["plugins", "skills"]`。插件贡献的 tools 在会话中投影为 `pluginFunctionCall` item；它们不会再生成 companion `toolCall` / `toolResult` item。面向用户的插件模型见 [插件与工具](../features/plugins-tools.md)。

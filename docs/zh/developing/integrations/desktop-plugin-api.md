...
本页是受信任 Desktop Plugin 的 API 参考。若要创建并构建第一个插件，请先阅读 [构建 Desktop Plugin](./desktop-plugins)。

## 使用四个内核元语

Runtime 只有四个内核元语。六类 contribution 为常见产品集成提供简洁 shortcut。

### 管理副作用

如果一项工作需要 setup 和 cleanup，但没有自然的 React owner，请使用 `host.effect`：
...
Effects、Host-owned subscriptions，以及通过其他元语创建的 registrations，都属于当前 revision generation。禁用、卸载、替换 revision 或关闭 Desktop 时，它们会一起清理。

### 组合 UI surface

根据需要进行的改变选择三种 `host.ui` 操作：
...
每次调用都会返回 disposable registration，同时也由 generation 管理。Dispose 一个 `add` 只会移除该项；dispose 一个 `replace` 会显露下一个 active replacement；dispose 一个 `wrap` 会用剩余 wrappers 重新组成链条。
...
`use` 返回最后一个 active provider 的同步快照；dispose 它以后会恢复前一个 provider。Desktop 模块可能并行激活，因此应在交互真正需要 service 时再解析，并处理 `undefined`。Manifest dependencies 要求同时存在 .NET，也不会让 Desktop provider 优先激活。Renderer service 不会自动跨入 .NET、CLI、远程客户端或 AppServer。
...
Event listener 会随 generation 一起移除。Event 只存在于 renderer，不会持久化 Session data，也不会变成 AppServer notification。
...
| Surface | 位置 |
|---|---|
| **`app`** | 完整的 Desktop application renderer tree。 |
| **`app.background`** | application shell 后方的空装饰 seat；Core background 仍位于 `app` 内。 |
...
Core 会把正常组件作为每个 surface 的 default content。`add` 在其后渲染内容；如需在前面加入内容并保留原行为，请使用 `wrap`；如需移除 Core 组件并自行接管替代行为，请使用 `replace`：
...
同一组名称会挂载在 thread、Welcome、approval 与 user-input Composer 中。即使当前 provider、compact mode、minimal chrome 或 decision state 隐藏了 Core default，surface 仍然可用。渲染插件内容前应检查共享 Composer context。Surface name 与 typed context 属于公共契约；surface 生成的 DOM 不属于公共契约。

只要 Composer 尚未创建或挂接到真实 Session thread，其 surface context 的 `threadId` 就为 `null`，这包括 welcome 与 detached embedded Composer；挂接后则为真实 thread id。

| 字段 | 含义 |
|---|---|
| `workspacePath` | 当前 workspace path；不可用时为 `null`。 |
| `threadId` | 已连接的 Session thread；连接前为 `null`。 |
...
在新聊天 Welcome 页，`composer` 覆盖完整的 pre-thread composition experience：app 选择、hero、输入框、workspace footer 与 quick starts。替换它时，这组共享 draft 与 voice lifecycle 的内容会作为一个整体切换。
...
Core 继续管理 mascot 的位置、bubble、menu、click handling、sleep timer、Composer handoff 与 outer motion。Context 继承普通 Composer 字段，并增加 `activity`、`expression`、`light`、`size`、`submitRevision`、`reasoningEffort`、`speed`、`contextMax` 与 `reducedMotion`。直接响应这些 snapshot；同一状态下连续提交需要 one-shot animation 时，请监听 `submitRevision`。插件自定义 occurrence 继续使用 `host.events`。
...
把自定义名称加入 `DesktopPluginSurfaceContextMap`，owner 与所有 consumer 才会获得同一个 context 类型。Provider 与 consumer package 应导入共享的 declaration module；没有 declaration merging 时，未知 surface 的 context 类型是 `unknown`。另一个已启用插件可以通过 `ui.add`、`ui.replace` 或 `ui.wrap` 定位 `acme-board.card.footer`，activation 顺序不影响注册。建议使用 plugin-qualified name，但不强制。

Surface 只在其组件 mounted 时存在。Registration 仍由注册它的 revision 持有，并会在 surface 存在时渲染。
...
这六个字段是 convenience API，不是 allowlist 或能力上限。通过 `host.ui.add("composer.toolbar.leading", ...)` 添加 Composer UI；如果功能不适合这些便捷字段，请使用 surface、service、event 与 effect。

返回的 activation 还可以提供 `dispose()`。Contribution id 在该 activation 中必须唯一。使用 `label.translations` 提供本地化标签；只有在 convenience API 定义了有序 placement 时才设置 `order`。

## 使用 Host API

除四个元语外，`DesktopPluginHost` 按 owner 对稳定的产品操作分组：

| 成员 | 用途 |
|---|---|
| `plugin`、`environment` | 插件身份，以及 locale、theme、platform 与 workspace context。 |
| `navigation` | View、Settings、thread、外部 URL 与用户通知。 |
...
从 `@dotcraft/plugin` 导入共享 UI 组件；官方 builder 会把 hooks 与 JSX 连接到 Desktop 的 React runtime。

Host API 是兼容性契约，不是 access-control boundary。主进程的 URL、route、bearer、size 与 timeout checks 是 service invariants，而不是插件权限。

## 有意识地使用 DOM 与 CSS

Desktop Plugin 可以访问 renderer DOM 并加载 global CSS，DotCraft 不会阻止这些方式；但 DotCraft-owned DOM structure、class names、private CSS variables、stores 与 feature components 都不是公共契约。已有公共 surface 或 service 时应优先使用；直接操作 DOM 或 CSS 需要自行承担维护成本。
...
请在 DotCraft 发现插件或打包插件之前完成构建。刷新插件列表，然后启用插件。Desktop 会把整个 content revision 作为一个 generation 激活。相同 revision 不发生变化；更新 revision 时，Desktop 会 dispose 旧 generation，再激活新 generation。

禁用或替换 revision 时，Desktop 会立即撤销 Host-owned registrations，不会等待插件尚未完成的 `activate()` 或 `dispose()` Promise；迟到的 activation result 也不能重新发布。

Revision 是开发迭代单元。Desktop Plugin 不内置 file watcher、HMR、component-only reload 或 partial-generation update。重新构建后，再刷新或重新启用插件。

Desktop 不会从远程 AppServer 加载可执行插件代码。使用远程 workspace 时，它只会激活本地已经打包，并且 plugin id、version 与 Desktop content revision 都和远程 snapshot 一致的代码。

## 相关文档

- [构建 Desktop Plugin](./desktop-plugins)
- [.NET 插件](./dotnet-plugins)

...
安装、启用、trust 与移除是独立的 AppServer 操作。安装不会授予 .NET trust；一次已经 `applied` 的变更仍可能让插件保持 `blocked`。方法、wire result、blocker 与 remediation data 见 [AppServer 协议](../protocols/appserver-protocol#plugins-和-skills-管理)。

## 相关文档

- [构建 .NET 插件](./dotnet-plugins)

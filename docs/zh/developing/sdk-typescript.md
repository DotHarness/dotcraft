...
`run()` 返回合并后的最终文本；`runStreamed()` 返回结构化事件并保留原始 JSON-RPC 消息。SDK 不会自动解析 `/command`、`$skill` 或 `@file` 文本；调用方应显式使用 `commandRefPart()`、`skillRefPart()`、`fileRefPart()` 或 raw command API。共享 SDK 事件拓扑见 [SDK](./sdk.md#事件拓扑)。
...
模块契约与宿主集成见 [TypeScript Module 集成](./typescript-module.md)。一方渠道包都依赖 `@dotcraft/sdk`，并从子路径导入：

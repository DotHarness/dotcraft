...
启用 `feishu.cli.enabled` 后，飞书来源的 Thread 会获得 `FeishuCli` 工具，每次调用都需要审批。适配器会使用配置的应用凭据换取并缓存 tenant access token，再以机器人身份运行内置 CLI。CLI 子进程只接收 App ID 和一个 access token，不接收 App Secret。

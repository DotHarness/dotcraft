...
- **AppServer** 把 ISessionService 投影成 JSON-RPC（[完整协议](../developing/appserver-protocol.md)），任何语言都可以做客户端。
...
- [AppServer 模式](../developing/appserver.md) — 手动启动、`--listen`、`--token`、远程连接
- [Hub 协议](../developing/hub-protocol.md) — 实现 Hub 客户端
- [AppServer 协议](../developing/appserver-protocol.md) — 实现 AppServer 客户端
...
|---|---|
| 普通 Desktop 用户 | 不需要 |
| 想多客户端共享同一工作区 | 阅读 [AppServer 模式](../developing/appserver.md) |
| 想构建机器人或外部渠道 | 阅读 [Python SDK](../developing/sdk-python.md) 或 [TypeScript SDK](../developing/sdk-typescript.md) |
| 想接入新 IDE / 编辑器 | 阅读 [ACP 模式](./entry-points/editors.md) |
| 想自定义协议层客户端 | 阅读 [AppServer 协议](../developing/appserver-protocol.md) |

## 相关入口

- [入口总览](./entry-points/index.md) — 现成入口对比与决策表
- [可观测性](./observability.md) — 在 Dashboard 看 Trace、审批、调用

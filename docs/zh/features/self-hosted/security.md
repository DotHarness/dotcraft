...
命令执行前，DotCraft 先确定由哪个 shell 来跑，再把脚本拆成一条条命令逐条检查。命令留在工作区内、又没有命中任何规则，就直接运行。以下情况会先征求你的同意：命令强制删除文件或打开网址，命令引用了工作区外的路径，或者你写的规则要求询问。DotCraft 不认识的 shell 名称会被直接拒绝。

...
命令执行前，DotCraft 先确定由哪个 shell 来跑，再把脚本拆成一条条命令逐条检查。命令留在工作区内、又没有命中任何规则，就直接运行。以下情况会先征求你的同意：命令强制删除文件或打开网址，命令引用或进入了工作区外的路径，命令切换到 DotCraft 无法判断的目录，或者你写的规则要求询问。DotCraft 不认识的 shell 名称会被直接拒绝。

有些命令会一直运行、等待后续输入，比如交互式 shell 或语言解释器的提示符。智能体往这类终端里输入的内容，会按和新命令同样的方式检查：对照该终端正在运行的 shell，以及 DotCraft 最后确认它所在的目录。一旦终端切换到 DotCraft 无法判断的目录，之后往里输入的任何内容都会先征求你的同意。

审批面板会显示将要使用的 shell、DotCraft 解读出的命令，以及询问的原因。脚本里有 DotCraft 无法提前读懂的语法时，面板会说明这一点，并把脚本整体展示出来。

...
| 场景 | 推荐 |
|---|---|
| 在已有对话中一次性委派独立任务 | [SubAgents](./subagents.md) |
| 用户请求需要规划、并行执行并产出统一答复 | Teams |
| 按计划或手动触发的工作 | [Automations](./automations.md) |
| 持续推进一条 Thread 朝长期目标演进 | [Goals](./automations.md#goals) |
...
运行时字段在 `teams` 配置段中。可用键见 [配置完整参考](../developing/configuration.md)。

## 相关入口

- [SubAgents](./subagents.md) — Teams 显得过重时的单级委派。
- [Automations 与 Goals](./automations.md) — 计划驱动的任务与 Thread 级目标，可与 Teams Mission 组合使用。
- [统一会话核心](./session-core.md) — 支撑 Mission 队友线程的 Thread / Turn / Item 模型。

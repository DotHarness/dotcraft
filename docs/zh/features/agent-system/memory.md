# 记忆与梦境

DotCraft 会把你教给它的东西带进后续会话：你说明过的偏好、你纠正过的做法，下次对话它还记得。

![DotCraft 记忆使用流程](/memory-lifecycle-topology.svg)

## 三种记忆方式

| 方式 | 适合什么 |
|---|---|
| **会话历史** | 回到之前的会话，接着上次的进度继续 |
| **已保存的记忆** | 保存你的偏好和纠正，每次会话自动带上 |
| **梦境** | 记录近期重点和开放问题这类还没定型的线索 |

## 管理记忆

直接告诉 DotCraft 要记住、更正或忘记什么，例如：“记住我喜欢简短的回答。”保存后，后续会话就能使用这些记忆。

不想使用记忆时，在 Desktop 中关闭 **设置 → 个性化 → 启用记忆**。之后新建的会话不再带记忆，梦境和欢迎页建议也会暂停，重新开启后恢复。

要清空项目记忆，打开 **设置 → 个性化 → 删除记忆**。这会同时清空梦境，保留会话记录。

## 梦境

梦境会回顾近期工作区活动，整理出供后续会话参考的暂定笔记。

在 Desktop 的 **设置 → 个性化 → 梦境** 中开启。每次运行的结果会先等你审阅，应用后才会被后续会话用到。想让运行结果直接生效，在 **管理** 里打开自动更新。

![梦境审阅流程](https://github.com/DotHarness/resources/raw/master/dotcraft/whats-new/dreams.gif)

## 相关文档

- [技能与自学习](./skills) — 记忆沉淀事实，技能沉淀做法：让成功的流程下次直接复用
- [可观测性](../self-hosted/observability) — 在 Dashboard 里回看梦境运行和会话轨迹

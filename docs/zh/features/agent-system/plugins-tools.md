...
连接远端工作区后，插件页面管理的是该工作区的插件。插件提供桌面界面时，DotCraft 会先询问是否允许该工作区在本机运行插件界面，确认后自动取得并加载所需文件。此授权也适用于该工作区之后安装的插件和更新。

插件仍安装在远端，不会在本地工作区重复安装。切换工作区会切换相应的插件界面。可在插件页面停止运行远端界面，或重试加载失败的界面，这不会卸载远端插件。

如果远端服务器需要更新才能支持插件界面，插件页面会提示你。该插件在远端的其他功能仍可使用。

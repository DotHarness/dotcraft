...
启用长期记忆沉淀后，每完成若干轮成功对话，DotCraft 会让 Agent 把这一段对话里产生的稳定信息写回 `.craft/memory/MEMORY.md`，并把流水追加到 `HISTORY.md`。具体触发频率和记忆整合模型见 [配置完整参考](../developing/configuration.md#workspace-memory-与-skills)。
...
- 在 [安全配置](./security.md) 里禁用相关工具，限制 Agent 写入工作区外路径。

## 相关入口

- [项目级工作区](./workspace.md) — `.craft/` 整体结构
- [Skills 与自学习](./skills.md) — 把成功流程沉淀为可复用 skill
- [Observability](./observability.md) — 在 Dashboard 审阅 Dreams、查看 Trace
- [配置完整参考](../developing/configuration.md) — `Memory.*` / `Compaction.*` 字段

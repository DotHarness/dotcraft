...
![DotCraft 对话中渲染的交互式天气可视化](https://github.com/DotHarness/resources/raw/master/dotcraft/whats-new/inline-visualizations.gif)

让 DotCraft 把对比、解释或结果做成交互式可视化，也可以直接调用 `$visualize`。视图会出现在回复中，不用离开对话就能操作和查看。

通过视图操作可以在确认问题内容后继续追问，也可以把当前状态复制为图片。
...
除了服务器，同事也可以把自己的电脑借给你。在**设置 → 连接 → 卫星**里完成配对，再到输入框的**执行位置**里选中它，当前对话的文件、Shell 和 LSP 工具就会在那台机器上执行，对话标题栏还能实时查看那台电脑的屏幕，详见 [DotCraft 卫星](../agent-system/satellite)。

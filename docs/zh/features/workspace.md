...
如果你不确定字段放哪：**Provider 放全局，工作区只覆盖项目相关选择**。Provider 字段、入口开关、Memory、Skills、Automations 和安全策略都统一放在 [配置完整参考](../developing/configuration.md)。
...
完整图文流程见 [快速开始](../getting-started.md)。
...
完整开关名、默认值和 JSON 示例见 [配置完整参考](../developing/configuration.md)。

## 相关入口

- [快速开始](../getting-started.md) — 第一次安装、配置、运行
- [长期记忆与 Dreams](./memory.md) — `MEMORY.md` / `HISTORY.md` 怎么自动维护
- [统一会话核心](./session-core.md) — 跨入口共享的 Thread/Turn/Item 模型
- [安全与沙箱](./security.md) — 工作区边界、黑名单、OpenSandbox
- [配置完整参考](../developing/configuration.md) — 所有字段

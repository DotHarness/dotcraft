...
Full role and profile configuration fields are in [SubAgent and External CLI Profiles](../developing/configuration.md#subagent-and-external-cli-profiles).
...
DotCraft passes role instructions to external CLIs but cannot enforce tool filtering inside the external CLI itself. For strong isolation, prefer `native` and combine it with role allow/deny lists and [Security & Sandbox](./security.md).
...
Custom external CLI profiles, resume extraction, permission forwarding, and vendor headless details are documented in [SubAgent and External CLI Profiles](../developing/configuration.md#subagent-and-external-cli-profiles).
...
- [Security & Sandbox](./security.md) — bound SubAgent behavior with workspace boundary and sandbox
- [Observability](./observability.md) — view SubAgent calls and approvals in Dashboard
- [Configuration Reference](../developing/configuration.md#subagent-and-external-cli-profiles)

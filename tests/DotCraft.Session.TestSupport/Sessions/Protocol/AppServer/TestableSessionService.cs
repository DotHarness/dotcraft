...
public class TestableSessionService : ISessionService, IThreadAgentRefreshService, ISubAgentSyntheticTurnService, ISubAgentThreadLifecycleService
{
    ...
    public Task<WorktreeCreateAndForkResult> CreateWorktreeAndForkAsync(
    ...
    public Task<WorktreeCreateAndStartResult> CreateWorktreeAndStartAsync(
        WorktreeCreateAndStartOptions options,
        CancellationToken ct = default)
        => throw new NotSupportedException("Worktree operations are not supported by this test service.");

    public Task<WorktreeHandoffResult> HandoffThreadWorktreeAsync(
        WorktreeHandoffOptions options,
        CancellationToken ct = default)
        => throw new NotSupportedException("Worktree operations are not supported by this test service.");

    public Task<IReadOnlyList<ThreadWorktreeStatus>> ListWorktreesAsync(
        SessionIdentity? identity = null,
        CancellationToken ct = default)
        => throw new NotSupportedException("Worktree operations are not supported by this test service.");

    public Task<ThreadWorktreeStatus> GetWorktreeStatusAsync(
        string threadId,
        CancellationToken ct = default)
        => throw new NotSupportedException("Worktree operations are not supported by this test service.");
    ...
    public async Task TryStartNextQueuedTurnAsync(string threadId, CancellationToken ct = default)
    {
        ...
        LastSubmittedContent = queued.MaterializedInputParts
            .Select(static part => part.ToAIContent())
            .ToList();
    }

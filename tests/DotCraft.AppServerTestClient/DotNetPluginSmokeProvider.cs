...
internal static class DotNetPluginSmokeProvider
{
    ...
    public static bool TryResolve(
        ...
        out string? errorCode)
    {
        var probeRoot = Path.Combine(workRoot, "provider-probe");
        ...
        try
        {
            ...
            var providerId = explicitProviderId ?? config.ProviderId?.Trim();
            ...
            var model = ResolveModel(config, providerId, explicitModel, authMethod);

...
internal sealed class DotnetPluginSmokeRunner(string dotcraftBin, DotnetPluginSmokeCliOptions options)
{
        ...
        DotnetPluginSmokeProviderSelection provider)
    {
        ...
        var toolCalls = new List<(string CallId, string Text)>();
        ...
        while (DateTimeOffset.UtcNow < deadline)
        {
            ...
            if (method == DotCraft.Protocol.AppServer.AppServerMethodNames.ItemCompleted)
                ReadCompletedItem(notification.RootElement, turnId, toolCalls, toolResults);
            ...
            {
                ReadCompletedTurn(notification.RootElement, toolCalls, toolResults);
                completed = true;
                break;
            }
        ...
        if (!string.Equals(call.Text, input, StringComparison.Ordinal))
            throw DotnetPluginSmokeProtocol.Failure("model-turn", "tool_arguments_mismatch");
        if (!toolResults.TryGetValue(call.CallId, out var result))
            throw DotnetPluginSmokeProtocol.Failure("model-turn", "tool_result_missing");
        if (!result.Contains(nonce, StringComparison.Ordinal)
            || !result.Contains(ReviewStamp, StringComparison.Ordinal))
        {
            throw DotnetPluginSmokeProtocol.Failure("model-turn", "tool_result_mismatch");
        }
    }

    internal static void ReadCompletedItem(
        JsonElement notification,
        string turnId,
        ICollection<(string CallId, string Text)> calls,
        IDictionary<string, string> results)
    {
        ...
        ReadItem(item, calls, results);
    }

    internal static void ReadCompletedTurn(
        JsonElement notification,
        ICollection<(string CallId, string Text)> calls,
        IDictionary<string, string> results)
    {
        ...
        IDictionary<string, string> results)
    {
        ...
        if (type == "toolCall")
        {
            if (payload.GetProperty("namespace").GetString() != "review"
                || payload.GetProperty("toolName").GetString() != "normalize"
                || payload.GetProperty("providerFlatName").GetString() != "review__normalize"
                || payload.GetProperty("source").GetProperty("pluginId").GetString() != ConsumerId)
            {
                throw DotnetPluginSmokeProtocol.Failure("model-turn", "unexpected_tool_provenance");
            }
            calls.Add((
                payload.GetProperty("callId").GetString() ?? string.Empty,
                payload.GetProperty("arguments").GetProperty("text").GetString() ?? string.Empty));
        }
        else if (type == "toolResult")
        {
            var callId = payload.GetProperty("callId").GetString() ?? string.Empty;
            if (!payload.GetProperty("success").GetBoolean())
                throw DotnetPluginSmokeProtocol.Failure("model-turn", "tool_result_failed");
            results[callId] = payload.GetProperty("result").GetString() ?? string.Empty;
        }
    }

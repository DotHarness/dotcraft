...
public sealed class SatelliteRegistryTests : IDisposable
{
    ...
    public void CreateInvite_ThenConsume_Once()
    {
        ...
        var (inviteId, expiresAt) = registry.CreateInvite("Ann", TimeSpan.FromHours(24));
    ...
    public void ConsumeInvite_AfterExpiry_Fails()
    {
        ...
        var (inviteId, _) = registry.CreateInvite("Ann", TimeSpan.FromMilliseconds(-1));
    ...
    {
        var registry = NewRegistry();
        var (inviteId, _) = registry.CreateInvite("Ann", TimeSpan.FromHours(24));

...
internal static class PluginContentTree
{
    ...
    public static string Fingerprint(
        ...
        Process(sourceRoot, destinationRoot: null, prefix, includeEntry);

    public static string CopyAndFingerprint(
        string sourceRoot,
        string destinationRoot,
        byte[] prefix,
        Func<string, bool>? includeEntry = null) =>
        Process(sourceRoot, destinationRoot, prefix, includeEntry);
        ...
        Func<string, bool>? includeEntry)
    {
        ...
        foreach (var entry in entries)
        {
            ...
            var include = includeEntry?.Invoke(normalizedPath) ?? true;
            if (entry.IsDirectory)
            {
                ...
                if (destinationRoot != null && include)
            ...
            using var destination = destinationRoot == null || !include
                ? null

...
internal sealed class ReasoningEffortJsonConverter : JsonConverter<ReasoningEffort>
{
    public override ReasoningEffort Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        ...
        {
            return Enum.IsDefined(typeof(ReasoningEffort), numeric)
                ? (ReasoningEffort)numeric
                : throw new JsonException($"Unknown reasoning effort numeric value: {numeric}.");
        }
        ...
        throw new JsonException("Reasoning effort must be a string or number.");
    }
...
internal sealed class ReasoningOutputJsonConverter : JsonConverter<ReasoningOutput>
{
    public override ReasoningOutput Read(ref Utf8JsonReader reader, Type typeToConvert, JsonSerializerOptions options)
    {
        if (reader.TokenType == JsonTokenType.Number && reader.TryGetInt32(out var numeric))
        {
            return Enum.IsDefined(typeof(ReasoningOutput), numeric)
                ? (ReasoningOutput)numeric
                : throw new JsonException($"Unknown reasoning output numeric value: {numeric}.");
        }
        ...
        throw new JsonException("Reasoning output must be a string or number.");
    }

...
public sealed class SdkRpcBindingsGenerator : IIncrementalGenerator
{
    ...
    private static string Generate(string json)
    {
        ...
        foreach (var method in methods)
        {
            ...
            if (method.Direction == "clientToServer" && method.Kind == "request")
            {
                source.Append("\n    /// <summary>Sends the <c>").Append(method.WireName).Append("</c> request.</summary>\n")
                    ...
                    .Append("        client.RequestAsync(global::DotCraft.Protocol.AppServer.AppServerRpc.").Append(method.DescriptorMember).AppendLine(", parameters, cancellationToken);");
            }
            else if (method.Direction == "clientToServer")
            {
                source.Append("\n    /// <summary>Sends the <c>").Append(method.WireName).Append("</c> notification.</summary>\n")
                    ...
                    .Append("        ").Append(paramsType).AppendLine(" parameters,")
                    .AppendLine("        global::System.Threading.CancellationToken cancellationToken = default) =>")
                    .Append("        client.NotifyAsync(global::DotCraft.Protocol.AppServer.AppServerRpc.").Append(method.DescriptorMember).AppendLine(", parameters, cancellationToken);");
            }
            else if (method.Kind == "request")
            {
                source.Append("\n    /// <summary>Registers a typed handler for <c>").Append(method.WireName).Append("</c>.</summary>\n")
                    ...
                    .Append("        client.RegisterServerRequestHandler(global::DotCraft.Protocol.AppServer.AppServerRpc.").Append(method.DescriptorMember).AppendLine(", handler);");
            }
            else
            {
                source.Append("\n    /// <summary>Registers a typed handler for <c>").Append(method.WireName).Append("</c>.</summary>\n")
                    ...
                    .Append("        client.RegisterNotificationHandler(global::DotCraft.Protocol.AppServer.AppServerRpc.").Append(method.DescriptorMember).AppendLine(", handler);");
            }
        }
        ...
        source.AppendLine("        {");
        source.AppendLine("            var parameters = notification.Params.Deserialize<TParams>(global::DotCraft.Protocol.AppServerContractJson.Options)");
        ...
        source.AppendLine("        {");
        source.AppendLine("            throw new ProtocolViolationException($\"Notification '{notification.Method}' did not match its Contracts DTO.\", exception);");
        source.AppendLine("        }");
        source.AppendLine("    }");
        source.AppendLine();
    ...
    private static string TypeName(string manifestType)
    {
        ...
        return name == "RpcEmpty"
            ? "global::DotCraft.Protocol.RpcEmpty"
            : "global::DotCraft.Protocol.AppServer." + name;
    }

...
10. A subscription target is the tuple of foreground workspace identity, connection epoch, and `threadId`. Desktop may start the subscription only after the target thread is present in the authoritative thread list for that foreground workspace.

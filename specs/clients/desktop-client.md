...
- App-bound Dynamic Tools may provide declarative Tool Result Presentation cards. Desktop renders them only through its trusted local block renderers, per [Tool Result Presentation](../protocols/tool-result-presentation.md).
- Supported presentation cards may include explicit capability actions such as opening the bound app, opening an `https:` URL, copying a value, approving or rejecting an app proposal, enqueuing a normal DotCraft turn, or making a read-only authorized app request (`app.request`).
- Card actions must not invoke agent/dynamic tools (`tool.call` is not an action kind), and must not execute shell commands, local executables, or arbitrary protocols. Human-initiated writes go through DotCraft-mediated `app.request` to the app's API, gated by binding scope, risk, and declared card-callable routes.

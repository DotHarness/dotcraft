...
- When `capabilities.sourceControlManagement` is available, Desktop exposes a workspace-scoped `Source Control` tab:
  ...
  - `Checkout` calls `sourceControl/changelist/prepare` and never invokes local `window.api.git.commit`; the dialog and toast copy must avoid submit/commit semantics. Desktop does not expose Perforce submit or shelve in this version.

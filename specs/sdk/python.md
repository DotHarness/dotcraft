...
class ThreadManager:
    ...
    async def list_turns(self, thread_id: str, cursor=None, limit=None, sort_direction=None) -> ThreadTurnsListResult: ...
    async def list_items(self, thread_id: str, turn_id=None, cursor=None, limit=None, sort_direction=None) -> ThreadItemsListResult: ...
```
...
class DotCraftThread:
    ...
    async def list_turns(self, cursor=None, limit=None, sort_direction=None) -> ThreadTurnsListResult: ...
    async def list_items(self, turn_id=None, cursor=None, limit=None, sort_direction=None) -> ThreadItemsListResult: ...
...
The handle caches the latest Thread header and updates it on lifecycle notifications; `refresh()` forces a header re-read. Persisted Turns and Items are read separately through bounded history pages. `run_streamed()` must avoid duplicate event delivery when an active subscription already exists for the thread, per the AppServer at-most-once rule.

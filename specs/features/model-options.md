...
Agent Profiles use the same complete `ModelPreference` contract rather than defining a second partial
model-options model:

- an omitted `providerPreference` captures the complete effective workspace/global provider preference
  when a new thread is created;
- a present `providerPreference` stores a provider id plus the complete `ModelPreference` fields directly;

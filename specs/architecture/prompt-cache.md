...
When a native SubAgent session is bound to its direct parent, tracing captures the parent's latest `PromptCacheRequestShape` as an immutable fork anchor. The child's first request shape produces exactly one `SubAgentPrefixDiagnostic`. A `match` requires equal protocol, model, prompt-cache key, instructions, tools, and reasoning hashes, with every parent `inputItemHash` forming the complete starting prefix of the child input. Any component change, shortened input, or item divergence is a `mismatch`; a missing parent shape is `unavailable`. The event records only component hashes, request/attempt indexes, item counts, the matched prefix length, the first divergence index, and changed-field names. It never records request content and does not compare the complete `inputHash`, because a child may append role guidance and its initial task after the inherited prefix.

...
SubAgent child threads use normal session tool construction with a role-resolved invocation policy. Their model-visible tool schema remains aligned with the parent so a fork keeps a stable prompt prefix; role restrictions are enforced when tools execute. For a native full-history fork (`forkTurns=all`), Session Core snapshots inheritable client-owned tool bindings from the direct parent onto the child after history materialization and before the child's first model sampling. This includes Runtime Dynamic Tools and the Desktop Node REPL. Fresh (`none`) and bounded forks do not inherit startup-scoped client tool bindings. The snapshot is not a live parent-child link: later parent replacements do not update the child, and process recovery still requires the client to rebind runtime capabilities through the existing resume contract. `agentRole` is a role selector, not just display metadata. The built-in `default` role denies DotCraft SubAgent control tools, `explorer` permits a read-only exploration subset, and `worker` may invoke write/shell/web tools plus Agent control when the depth policy allows it. Workspace configuration may override or add roles.

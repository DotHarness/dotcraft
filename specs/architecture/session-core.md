...
SubAgent child threads use normal session tool construction with a role-resolved invocation policy. Their model-visible tool schema remains aligned with the parent so a fork keeps a stable prompt prefix; role restrictions are enforced when tools execute. `agentRole` is a role selector, not just display metadata. The built-in `default` role denies DotCraft SubAgent control tools, `explorer` permits a read-only exploration subset, and `worker` may invoke write/shell/web tools plus Agent control when the depth policy allows it. Workspace configuration may override or add roles.

...
| M6 | **`ThreadRuntime` aggregation** | 4.3.1: introduce registry + runtime objects; coordinators and SessionService migrate field-by-field; delete the 24 dictionaries. | Highest — concurrency-sensitive; do last, smallest reviewable steps | **In progress** — `ThreadRuntimeRegistry`/`ThreadRuntime` introduced; cached threads, queue/agent locks, agent/tool/MCP caches, mode managers, plugin/dynamic tool-name sets, materialization flag, prompt snapshots, and context-usage anchors migrated into runtime registry; teardown count assertion added. Continue remaining state migrations. |
...
>   context-usage anchors; remaining per-thread and per-turn dictionaries still need
>   field-cluster migrations into `ThreadRuntime`/`TurnRuntime`.

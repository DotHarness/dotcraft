...
>
> Current recommended sequence: continue M4 thick domains (`thread/*`, `turn/*`, `initialize`)
...
> - M4 thick (`thread/*`, `turn/*`, `initialize`) — coupled via `EnrichThreadWireAsync`,
>   `ChannelSessionScope`, `CreateChannelScope`, `NormalizeIdentityWorkspace`, `TurnStartCoordinator`.

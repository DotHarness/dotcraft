...
>
> **As-built (M3 batch, 8 domains done).** Extracted, each verified (Core build + 386 conformance
> tests, green per commit): `CronRequestHandler`, `TerminalRequestHandler`, `DreamsRequestHandler`,
> `SkillsRequestHandler`, `McpRequestHandler`, `ChannelRequestHandler`, `UsageRequestHandler`,
...
> - `provider/*` and `workspace/config*` — still edit/read
...
>   path. Provider/workspace extraction still needs the remaining config-refresh and
>   agent-invalidation seams grouped first.

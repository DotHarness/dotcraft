...
Status: **Active** (M1 done; M2 partial; M3 done; M4 in progress)
...
| M4 | **Domain handlers, batch 2** | thread, turn (+`TurnStartCoordinator`), worktree, subagent, initialize. Delete the old switch; `AppServerRequestHandler` reaches dispatcher-only form. | Medium (thick handlers) | **In progress** — `subagent/*` extracted to `SubAgentRequestHandler` with thread-enrichment and coordinator-factory seams. Remaining: `thread/*`, `worktree/*`, `turn/*`, `initialize`. |
...
>
> Current recommended sequence: continue M4 thick domains (`thread/*`, `worktree/*`, `turn/*`,
> `initialize`) and factor thread projection / turn-start orchestration
...
> - M4 thick (`thread/*`, `turn/*`, `worktree/*`, `initialize`) — coupled via

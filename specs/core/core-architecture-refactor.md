...
> - M6 `ThreadRuntime` aggregation — runtime registry owns cached `SessionThread`
>   instances, event brokers, queue/agent locks, agent/tool/MCP caches, mode managers,
>   plugin/dynamic tool-name sets, materialization flags, prompt snapshots,
>   context-usage anchors, maintenance state, auto-consolidation state, and per-turn
>   cancellation/approval/user-input/goal snapshots. Delete tombstones are centralized in
>   the registry so stale background turn cleanup cannot recreate a deleted runtime.

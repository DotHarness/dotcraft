...
- **`ui/open-link`**: host opens an `https:` or `mailto:` URL. **Scheme handling is host‑owned policy** — apps do **not** declare custom schemes (consistent with MCP Apps / OpenAI Apps SDK, which both leave scheme handling to the host). `javascript:` / `data:` / `file:` and all other schemes are forbidden. App deep‑links ("Open in Oratorio") route through the existing app‑handoff mechanism in M‑v, not through `ui/open-link`.
...
- **`ui/open-link` schemes** → **host‑owned scheme policy** (`https:`/`mailto:`); no per‑app declared schemes (consistent with MCP Apps and the OpenAI Apps SDK, neither of which lets servers declare custom schemes).
